...
- LLM: Groq (Llama 3.3 70B — chat/overview generation, free tier, no billing card required) + Gemini gemini-embedding-001 (embeddings). Generation moved off Gemini after every Gemini generateContent free-tier quota tested as a hard 0 across multiple keys/projects — see prod.md.
...
routes/v1/repository.route.ts — POST /repository/ingest (github), POST /repository/ingest/upload (zip, multer), GET /repository (list, bounded to 50 most recent), GET /repository/:id
routes/v1/chat.route.ts — POST /chat/:repositoryId/message (404 if repo not found, 409 if status !== 'ready'), GET /chat/:repositoryId/history. User message is saved immediately, before retrieval/generation — a downstream LLM failure never silently drops it from history; a generic assistant error message is also persisted on failure so the transcript shows something happened.

## Services (add one line per service as features are built)
services/repository.service.ts — createGithubRepository, createUploadRepository, setRepositoryJobId, updateRepositoryStatus, getRepositoryById, listRepositories(limit = 50) — bounded, ordered by createdAt desc
services/ingestion.service.ts — cloneRepository (simple-git shallow clone), extractZip (adm-zip), walkFiles (recursive walk, skips node_modules/dist/build/.next only — real source is never filtered; extraction itself is always unmodified 1:1), removeWorkingDir
...
services/generation.service.ts — generateText(prompt) — provider-switched (config/llm.config.ts GENERATION_PROVIDER, default 'groq'): generateWithGroq (fetch to Groq's OpenAI-compatible /chat/completions) or generateWithGemini (fallback, kept for parity). Returns { text, usage: TokenUsage | null } normalized across both providers.
...
services/prompt.service.ts — buildRetrievalContext(repositoryId, query) (fetches repo overview always; skips embedding the query + chunk retrieval entirely when ENABLE_CHUNKING_EMBEDDING=false — no wasted embed_content call against zero rows), buildChatPrompt(question, context) (assembles final prompt text, truncates lowest-similarity chunks first if over MAX_CONTEXT_CHARS, wraps each chunk in <untrusted_code_context> delimiters + instructs the model never to treat retrieved content as commands — basic prompt-injection resistance, not a guarantee)
...
services/guardrails.service.ts — shouldRefuseForInsufficientContext(context: ChatContext) — never refuses if a repository overview exists (overview alone is enough context to attempt an answer); otherwise true if no chunks retrieved or best similarity < MIN_SIMILARITY_THRESHOLD. buildRefusalResponse() (canned refusal, no LLM call made — real cost/latency win for clearly-irrelevant questions, not just a text hedge)
...
config/embedding.config.ts — MAX_CHUNKS_PER_REPOSITORY (default 200), EMBEDDING_BATCH_SIZE (default 40) — tuned so a full ingestion is ~5 embedding requests, under Gemini free-tier's 100/min cap, EMBEDDING_MODEL_NAME (default gemini-embedding-001) — all env-overridable. EMBEDDING_OUTPUT_DIMENSIONS (768, not env-overridable — must match CodeChunk.embedding's fixed vector(768) column). MAX_CHUNKS_PER_REPOSITORY is the knob for staying under the embedding provider's free-tier rate limit.
config/llm.config.ts — GENERATION_PROVIDER (default 'groq'), GENERATION_MODEL_NAME (default llama-3.3-70b-versatile), GROQ_API_KEY, MAX_KEY_FILES_FOR_OVERVIEW (default 8), MAX_KEY_FILE_CHARS (default 2000) — bounds the repository overview prompt size regardless of repo size.
...
## Middleware
middleware/upload.middleware.ts — uploadZipMiddleware (multer, .zip only, 200MB limit, disk storage to tmp/uploads/)
...
MAX_CHUNKS_PER_REPOSITORY — optional, default 200
EMBEDDING_BATCH_SIZE — optional, default 40
EMBEDDING_MODEL_NAME — optional, default gemini-embedding-001
ENABLE_CHUNKING_EMBEDDING — optional, default true. Set to "false" to skip chunking/embedding entirely and rely on the repository overview only (demo/testing toggle, see config/embedding.config.ts)
GENERATION_PROVIDER — optional, default groq
GENERATION_MODEL_NAME — optional, default llama-3.3-70b-versatile
